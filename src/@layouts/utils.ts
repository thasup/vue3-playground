...
/**
 * Return boolean if current child route match menu
 * @param {Object, String} item navigation routeName or route Object provided in navigation data
 */
export const isChildActive = computed(() => (link: NavLink) => {
  const route = useRoute()
  const routeName = route.name as string
  const menuLink = link.to as string

  return routeName !== menuLink && routeName.startsWith(menuLink)
})
